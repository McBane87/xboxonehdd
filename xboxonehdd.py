...
def print_parted_commands(device):
    ...
    f.write('#!/bin/bash\n')
    f.write('DEV={0}\n'.format(device))
    f.write('parted -s "$DEV" mktable gpt; sleep 2\n')
    f.write('parted -s "$DEV" mkpart primary ntfs 1.00MiB {0}MiB; sleep 2\n'.format(temp_end))
    f.write('parted -s "$DEV" name 1 "\\"Temp Content\\""; sleep 2\n')
    f.write('mkntfs -q "${DEV}1" -f -L "Temp Content"; sleep 2\n')
    f.write('parted -s "$DEV" mkpart primary ntfs {0}MiB {1}MiB; sleep 2\n'.format(temp_end, user_end))
    f.write('parted -s "$DEV" name 2 "\\"User Content\\""; sleep 2\n')
    f.write('mkntfs -q "${DEV}2" -f -L "User Content"; sleep 2\n')
    f.write('parted -s "$DEV" mkpart primary ntfs {0}MiB {1}MiB; sleep 2\n'.format(user_end, sys_end))
    f.write('parted -s "$DEV" name 3 "\\"System Support\\""; sleep 2\n')
    f.write('mkntfs -q "${DEV}3" -f -L "System Support"; sleep 2\n')
    f.write('parted -s "$DEV" mkpart primary ntfs {0}MiB {1}MiB; sleep 2\n'.format(sys_end, upt_end))
    f.write('parted -s "$DEV" name 4 "\\"System Update\\""; sleep 2\n')
    f.write('mkntfs -q "${DEV}4" -f -L "System Update"; sleep 2\n')
    f.write('parted -s "$DEV" mkpart primary ntfs {0}MiB {1}MiB; sleep 2\n'.format(upt_end, upt2_end))
    f.write('parted -s "$DEV" name 5 "\\"System Update 2\\""; sleep 2\n')
    f.write('mkntfs -q "${DEV}5" -f -L "System Update 2"; sleep 2\n')
    f.flush()
    f.close()
